...
router.post('/',[
    body('name').isLength({ min: 5, max: 20 }).withMessage('La longueur doit être entre 5 et 20 caractères pour le name de la catégorie').bail().trim().escape(),
...
], handleValidationErrors,updateCategory);

router.delete('/:id',[  
    param('id').isInt().withMessage(`L'id de la catégorie est invalide`)
], handleValidationErrors,deleteCategory);
router.get('/:id',[  
    param('id').isInt().withMessage(`L'id de la catégorie est invalide`)
], handleValidationErrors,getCategoryById);
